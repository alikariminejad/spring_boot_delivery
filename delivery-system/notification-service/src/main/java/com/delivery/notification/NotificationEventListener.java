...
public class NotificationEventListener {
    ...
    public void handleOrderPlaced(OrderPlacedEvent event) {
        ...
        notificationService.createNotification(event.getCustomerUsername(), message, NotificationType.ORDER_PLACED.toString(), event.getOrderId());
    }

    @RabbitListener(queues = RabbitMQConfig.NOTIFICATION_QUEUE)
    public void handleCourierAssigned(CourierAssignedEvent event) {
        String message = "Order " + event.getOrderId() + " assigned to you.";
        notificationService.createNotification(event.getCourierUsername(), message, NotificationType.COURIER_ASSIGNED.toString(), event.getOrderId());
    }

    @RabbitListener(queues = RabbitMQConfig.NOTIFICATION_QUEUE)
    public void handleOrderStatusChanged(OrderStatusChangedEvent event) {
        String message = "Order " + event.getOrderId() + " status changed from " + event.getOldStatus() + " to " + event.getNewStatus();
        notificationService.createNotification(event.getCustomerUsername(), message, NotificationType.STATUS_CHANGED.toString(), event.getOrderId());
        if (event.getCourierUsername() != null) {
            notificationService.createNotification(event.getCourierUsername(), message, NotificationType.STATUS_CHANGED.toString(), event.getOrderId());
        }
    }
    ...
    public void handleSettlementApproved(SettlementApprovedEvent event) {
        ...
        notificationService.createNotification(event.getCourierUsername(), message, NotificationType.SETTLEMENT_APPROVED.toString(), event.getSettlementId());
    }
    ...
    public void handleSettlementRejected(SettlementRejectedEvent event) {
        ...
        notificationService.createNotification(event.getCourierUsername(), message, NotificationType.SETTLEMENT_REJECTED.toString(), event.getSettlementId());
    }
}
